...
%%基础参数
N = 100;  %种群内个体数目
N_chrom = inputnum*hiddennum+hiddennum+hiddennum*outputnum+outputnum; %染色体节点数
iter = 50; %迭代次数
mut = 0.2;  %突变概率
acr = 0.2; %交叉概率
best = 1;

chrom_range = [-3*ones(N_chrom,1) 3*ones(N_chrom,1)]'; %每个节点的值的区间
...
for i=1:N
    fitness(i)= CalFitness(chrom,inputnum,hiddennum,outputnum,net,inputn,outputn); %计算适应度
end
